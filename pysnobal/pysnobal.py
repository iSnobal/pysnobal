...
def run_snobal(
    ...
    forcing_data_df, mh, params, tstep_info, output_rec = _parse_inputs(forcing_data_df, config)
    ...
    dt_dic = {'Datetime' : []}  # TODO: can use dictionary intersection operator to make more elegant in more recent versions of python
    eb_dic = {defaults.OUTPUT_NAMES_SNOBAL2CUSTOM[col]: [] for col in defaults.EM_OUT}
    snow_dic = {defaults.OUTPUT_NAMES_SNOBAL2CUSTOM[col]: [] for col in defaults.SNOW_OUT}
    ...
    for override in overrides:
        ...
        for k in keys[:-1]:
            ...
            if subconfig is None:
                raise ValueError(f"Invalid override '{key}'. Parameter not accepted in config.")
...
def _load_override_config() -> dict[str, Any]:
    ...
    parser = argparse.ArgumentParser(description='Run Snobal using the forcing data and model parameters in config. Optionally provide overrides to config as args.')
    parser.add_argument('--config', '-c', type=str, required=True, help='Path to YAML config file.')
    parser.add_argument('--override', '-o', nargs='*', default=[], help='Override config values, e.g. -o io.forcing_path=./cssl_wy17_forcing.csv params.elevation=2101')
...
def _check_forcing_df(forcing_data_df: pd.DataFrame) -> float:
    ...
    for k in defaults.FORCING_NAMES_CUSTOM2SNOBAL.keys():
        if k not in forcing_data_df.columns:
            raise ValueError(f'Dataframe missing {k}. Dataframe must contain the following columns: {list(defaults.FORCING_NAMES_CUSTOM2SNOBAL.keys())}')
        
    # calculate timestep frequency in seconds
    timesteps = np.unique(np.diff(forcing_data_df.index)).astype('timedelta64[s]').astype(float)  # TODO can make more readable using .diff() if using more recent version of python
    if len(timesteps) > 1:
        raise ValueError(f'Dataframe has a non-uniform timestep. Found the following timesteps: {timesteps}')

    data_tstep_sec = timesteps[0]

    # verify the inputs are serially complete (no NaNs)
    for col, nan in forcing_data_df[defaults.FORCING_NAMES_CUSTOM2SNOBAL.keys()].isna().sum().items():
        if nan > 0:
            raise ValueError(f'Column {col} is not serially complete (i.e. contains NaN)')
...
def _check_config(config: dict[str, Any], data_tstep_sec: float) -> None:
    ...
    if config['io'].get('output_path') is None:
        raise ValueError("config must contain a path for the output data: {'io' : {'output_path' : <your path>}}")
    ...
    for h in ['air_temp_m', 'soil_temp_m', 'wind_speed_m']:
        if config['z'].get(h) is None:
            raise ValueError(f"config must contain the instrument height/depth for {h}: {{'z' : {{{h} : <instrument height meters>}}}}")
    ...
    for p in ['elevation_m', 'roughness_length_m']:
        if config['params'].get(p) is None:
            raise ValueError(f"config must contain specify the value for {p}: {{'params' : {{{p} : <value>}}}}")
    
    # backfill model parameters with defaults
    if (config.get('init') is None) or all(value is None for value in config['init'].values()):
        config['init'] = defaults.DEFAULT_SNOWPACK
    else:
        for s in []:
            if config['init'].get(s) is None:
                raise ValueError(f"if specifying the initial snowpack state in config, config must contain {s}: {{'init' : {{{s} : <value>}}}}")
...
def _parse_inputs(
    ...
    mh = {'z_t': config['z']['air_temp_m'], 'z_g' : config['z']['soil_temp_m'], 'z_u' : config['z']['wind_speed_m']}

    # assemble parameter dictionary
    params = {'relative_heights' : config['defaults']['relative_heights'] , 
    ...
    tstep_info = [{'level': 0, 'output': 0x2, 'threshold': None, 'time_step': data_tstep_sec, 'intervals': None},
                  {'level': 1, 'output': False, 'threshold': config['defaults']['normal_tstep_mass_thresh_kgm-2'], 'time_step': _min2sec(config['defaults']['normal_tstep_min'])},
                  {'level': 2, 'output': False, 'threshold': config['defaults']['medium_tstep_mass_thresh_kgm-2'], 'time_step': _min2sec(config['defaults']['medium_tstep_min'])},
                  {'level': 3, 'output': False, 'threshold': config['defaults']['small_tstep_mass_thresh_kgm-2'], 'time_step': _min2sec(config['defaults']['small_tstep_min'])}
    ]

    for i in range(1, 4):
        tstep_info[i]['intervals'] = int(tstep_info[i - 1]['time_step'] / tstep_info[i]['time_step'])
    ...
    for s in config['init']:
        output_rec[defaults.PARAM_NAMES_CUSTOM2SNOBAL[s]] = np.atleast_2d(config['init'][s])

    return forcing_data_df, mh, params, tstep_info, output_rec

def _append_output(
    running_output: dict[str, list], 
    dt: pd.Timestamp, 
    ...
    for x in defaults.EM_OUT:
        running_output[defaults.OUTPUT_NAMES_SNOBAL2CUSTOM[x]].append(output_rec[x][0][0])
...
def main():
    ...
    # open forcing data 
    forcing_data_df = pd.read_csv(config['io']['forcing_path'], parse_dates=True, index_col=0)
